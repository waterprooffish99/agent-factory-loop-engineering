...
REPO = os.path.abspath(os.path.join(HERE, "..", "..", "..", ".."))  # project-7 root
SPINE = os.path.join(REPO, "progress.md")   # the memory file — read first, written last
BRIEFS_DIR = os.path.join(REPO, "briefs")
REQUIRED_INPUT = os.path.join(REPO, "inputs", "daily-source.md")
FAILURE_GUARD = os.path.join(REPO, ".project7-failure-attempted")
MAX_FAILURE_ATTEMPTS = 1
HUMAN_ACTION = "Create or restore inputs/daily-source.md, then review it before retrying."
...
def claim_failure_attempt(now):
    """Atomically allow this controlled failure to be attempted only once."""
    try:
        fd = os.open(FAILURE_GUARD, os.O_WRONLY | os.O_CREAT | os.O_EXCL, 0o600)
    except FileExistsError:
        return False
    with os.fdopen(fd, "w", encoding="utf-8") as f:
        f.write(f"attempts={MAX_FAILURE_ATTEMPTS}\nclaimed={now.strftime('%Y-%m-%dT%H:%M:%SZ')}\n")
    return True


def record_operational_failure(state, beat, now, operation, error):
    """Leave enough evidence to diagnose the failed beat without replaying it."""
    timestamp = now.strftime("%Y-%m-%dT%H:%M:%SZ")
    reason = f"{type(error).__name__}: {error}"
    state["open"].append(
        f"- [failure:beat-{beat}] {timestamp} | failed operation: {operation} "
        f"| error reason: {reason} | status: NEEDS_HUMAN "
        f"| suggested human action: {HUMAN_ACTION}"
    )
    state["checkpoints"].append(
        f"- {timestamp} | beat {beat} | new: 0 | escalated: 1 "
        f"| status: NEEDS_HUMAN | failed operation: {operation} | error reason: {reason}"
    )
    write_spine(state)
    print(f"  beat {beat}: operational failure")
    print(f"  timestamp: {timestamp}")
    print(f"  failed operation: {operation}")
    print(f"  error reason: {reason}")
    print("  status: NEEDS_HUMAN")
    print(f"  suggested human action: {HUMAN_ACTION}")
    print("  spine: progress.md updated with failure evidence")
    return 1
...
def run_beat():
    ...
    if not claim_failure_attempt(now):
        print(f"  failure guard: maximum attempts ({MAX_FAILURE_ATTEMPTS}) already used; refusing to run")
        return 3
    operation = "read required input inputs/daily-source.md"
    try:
        with open(REQUIRED_INPUT, encoding="utf-8") as f:
            f.read()
    except OSError as error:
        return record_operational_failure(state, beat, now, operation, error)
